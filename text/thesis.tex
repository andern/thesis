...
time-consuming. A major part of the third main function is a call to a QP
solver\cite{trond}.
...
\begin{equation}
f(x) = -g - x^T \Phi D x + b^T x, \label{eq:goodtech}
\end{equation}
...
($E/J$), and $b$ is a vector containing specific power delivery price
($E/J$). The symbols in parentheses are units of measurements.
The symbol $E$ is the monetary unit, $J$ is joule and
$W$ is watt\cite{digernes}.

We simplify (\ref{eq:goodtech}) to
\begin{equation}
    f(x) = x^T H x + b^T x, \label{eq:obj}
\end{equation}
where $H$ is a diagonal matrix that represents quadratic cost terms ($E/s$).
sign\cite{digernes}.
...
We discuss breakdowns in more detail in Section \ref{sec:instances},
\ref{sec:subinstances} and Chapter \ref{ch:tree}.

In the next section, we present a short introduction to linear and quadratic
programming.
We then move on to Chapter \ref{ch:qp} where we present some quadratic
programming problems that Goodtech have supplied
Afterwards, in Chapter \ref{ch:slp}, we present
an iterative optimization method based on Successive Linear Programming (SLP).
When we move on to Chapter \ref{ch:tree}, we present methods for eliminating
unnecessary work carried out in the third main function of PROMAPS, and
thereby improving the speed in which it is performed.
...
We presented an iterative method based on linear programming, as the data
...
Goodtech to pay any license fees.
It is important
...
16\% to around 70\% faster, and with reason to believe that it would increase
even further, as $\beta$ increases. This reduced the average number of
...
simultaneously is of interest. We suggest the parallelization of the methods
